...
use Tie::IxHash;
use Bio::DB::Fasta;
# coded by Qing Li on 07/29/19.
# #Copyright (c) 2019 Huntsman Cancer Institute. All rights reserved.
#
#
# #-----------------------------------------------------------------------------
# #----------------------------------- MAIN ------------------------------------
# #-----------------------------------------------------------------------------
my $usage = "tABRA.pl fastqReads blastxDb pepLength bxCpus trinityCpus rsemCpus outName\n
...
unless(-f "$sample.trinity.Trinity.fasta" ){
	system("Trinity --seqType fq --max_memory 30G --bypass_java_version_check --single $fastq_reads --CPU $triCpus --bflyHeapSpaceMax 5G --bflyCPU $bflyCpus --KMER_SIZE 31 --SS_lib_type RF --min_kmer_cov 10 --min_glue 10  --output $sample.trinity --full_cleanup");
}
...
close OUT;
#to here!
system("align_and_estimate_abundance.pl --transcripts $sample.trinity.Trinity.rf.fasta --seqType fq --single $fastq_reads --SS_lib_type RF --est_method RSEM --aln_method bowtie2 --trinity_mode --prep_reference --output_dir $sample.rsem_outdir --thread_count $rsemCpus ");
